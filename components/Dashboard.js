  ...
  return (
    <div className='flex flex-col flex-1 gap-8 sm:gap-10 md:gap-16 '>
      ...
      <h4 className={'text-5xl sm:text-6xl md:text-7xl text-center ' + fugaz.className}>
        How do yo <span className='textGradient '>feel</span> today?
      </h4>
      <div className='flex items-strech flex-wrap gap-4   '>
        {Object.keys(moods).map((mood, moodIndex)=>{
          ...
          }} className={' px-5 p-4 rounded-2xl purpleShadow duration-200 bg-indigo-50 hover:bg-indigo-100 text-center flex flex-col gap-2 flex-1 items-center '
           + (moodIndex === 4 ?  ' col-span-2 sm:col-span-1' : ' ')} key={moodIndex}>
            <p className='text-4xl sm:text-5xl md:text-6xl'>{moods[mood]}</p>
            <p className={'text-indigo-500 text-xs sm:text-sm md:text-md ' + fugaz.className}>{mood}</p>
          </button>
          )
        })}
      </div>
